...
if __name__ == "__main__":
    ...
    alpha = (
        rank * 2
    )  # must be at least equal to rank, otherwise the weights won't update coz the scaling is alpha / rank
    dataset_path = download_file(args.url, args.dir)
    tokenizer = tiktoken.get_encoding("gpt2")
    config = OpenAIModelConfigs.gpt2_med_255m
    file_name = f"data/models/{re.sub(r'[^a-zA-Z0-9]+', '_', config.hf_repo_id) }-instruct-lora.pth"
    ...
    if os.path.exists(file_name):
        ...
        num_epochs = 2
        train_losses, val_losses, examples_seen = train_generator_simple(
            model=model,
            config=config,
        ...
        plt.savefig(f"data/plots/{file_name}-losses-plot.pdf")
    ...
    for entry in test_data[:3]:  # 1
        ...
        with open(f"{args.dir}/instruction-data-with-response-lora.json", "w") as file:
